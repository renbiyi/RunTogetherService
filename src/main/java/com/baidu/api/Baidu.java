...
public final class Baidu {
     ...
     * 
     * @param clientId 应用的appKey信息
     * @param clientSecret 应用的appSecret信息
     * @param redirectUri 应用设置的回调地址
     * @param store 提供存储Token信息的存储类
     * @param request HttServletRequest对象，用来获取cookie信息
     * @throws BaiduApiException
     * @throws BaiduOAuthException
     */
    public Baidu(String clientId, String clientSecret, String redirectUri, BaiduStore store,
            HttpServletRequest request) throws BaiduApiException, BaiduOAuthException {
        if (store == null) {
    ...
    private Session doGetSession() throws BaiduApiException, BaiduOAuthException {
        ...
        if (!StringUtil.isEmpty(tmpCode) && !tmpCode.equals(store.getCode())) {
            try {
                BaiduOAuthToken token = getBaiduOAuth2Service().getAccessTokenByAuthorizationCode(
                        tmpCode);
                String accessToken = token.getAccessToken();
                String jsonUser = getBaiduApiClientService(accessToken).request(LoggedInUser_URL,
                        null, "GET");
