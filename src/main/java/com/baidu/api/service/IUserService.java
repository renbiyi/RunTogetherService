...
public interface IUserService {
     ...
     * 返回指定用户的用户资料。
     * 
     * @param uid 用户id, 为空则表示当前登录用户
     * @param fields 期望获得的用户资料的字段列表，用逗号隔开，为空则获取userid, username, realname
     ...
     * 判定用户是否已经为应用授权。
     * 
     * @param uid 用户uid，为空则默认是当前用户
     * @param appid 应用的appid，为空则默认是当前应用
     ...
     * 
     * @param ext_perm 单个权限，例如接收email等，
     * @param uid 用户uid，为空则默认是当前用户。
     ...
     * 
     * @param ext_perms 多个权限半角逗号隔开，例如basic,email等
     * @param uid 用户uid，为空则默认是当前用户
